...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'view_installation_requests_policy'
  ) THEN
    CREATE POLICY "view_installation_requests_policy" ON installation_requests
      FOR SELECT
      USING (
        company_id IN (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'installation_requests' 
    AND policyname = 'manage_installation_requests_policy'
  ) THEN
    CREATE POLICY "manage_installation_requests_policy" ON installation_requests
      FOR ALL
